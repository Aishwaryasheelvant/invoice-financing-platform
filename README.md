...
Requires Docker and Docker Compose. No local Node, Postgres, or Redis install needed.
...
```bash
cp .env.example .env   # then edit JWT_*_SECRET at minimum
docker compose up --build
```
...
Same as Option B, but point `.env` at your local Postgres/Redis instead of starting the compose services.
...
npm run migration:run             # or point DB_DATABASE at the test DB and run migrations once
npm run test:e2e
```
