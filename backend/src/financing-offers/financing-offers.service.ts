...
export class FinancingOffersService {
  ...
  /**
   * The SME who owns the invoice sees every bid on it (they need the full
   * picture to decide which to accept). A financier sees only their own
   * bid — not competitors' terms, same as any real bidding process.
   */
  async listForInvoice(invoiceId: string, currentUser: AuthenticatedUser): Promise<OfferResponseDto[]> {
    ...
    if (currentUser.role === UserRole.FINANCIER) {
      return offers.filter((offer) => offer.financierId === currentUser.id).map(OfferResponseDto.fromEntity);
    }
    throw new ForbiddenException('Not allowed to view offers on this invoice');
  }
